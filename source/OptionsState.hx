...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		menuBG.frames = Paths.getSparrowAtlas('menucharacters/Menu_Skeletal');
		menuBG.animation.addByPrefix('funny','menuidle', 48, true);
		menuBG.screenCenter();
		menuBG.antialiasing = ClientPrefs.globalAntialiasing;
		menuBG.animation.play('funny');
		add(menuBG);
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	private function addBindTexts(optionText:Alphabet) {
		var text1 = new AttachedText(InputFormatter.getKeyName(controlArray[grpInputs.length]), 400, -55, true);
		text1.setPosition(optionText.x + 400, optionText.y - 55);
		text1.sprTracker = optionText;
		grpInputs.push(text1);
		add(text1);

		var text2 = new AttachedText(InputFormatter.getKeyName(controlArray[grpInputs.length]), 650, -55, true);
		...
		for (i in 0...options.length)
		{
			...
			if(!isCentered) {
				...
				if(useCheckbox) {
					...
					var valueText:AttachedText = new AttachedText('0', optionText.width + 80, null, true);
